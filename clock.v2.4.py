...
# Proper centring on clock screen

# v2.4
# Python 3.2
# Countdown display in menu
# Internet/wifi icon
# Anniversary "None" music option
...
# Python MPD
# Manual override day/night
# Bank holiday detect?
# Check is radio is streaming before backup alarm
...
button_day_menu = pygame.Rect(140, 140, 50, 50)
...
def get_data():
	...
	try:
		url="http://api.openweathermap.org/data/2.5/weather?q=Westerham,uk&APPID=d1a563a41a98799694adbb5b63e2c008"
		response=urlopen(url).read().decode('UTF-8')
		#response = urlopen('http://api.openweathermap.org/data/2.5/weather?q=Westerham,uk&APPID=d1a563a41a98799694adbb5b63e2c008')
	except Exception:
...
while 1:
		...
		if event.type is pygame.MOUSEBUTTONDOWN:
			...
			if alarm == "on":
				...
				if button_snooze.collidepoint(x, y):
					...
					snoozetime = timeout.strftime('%H:%M')
					alarm = "off"
					stop_music()
				...
				if button_status_menu.collidepoint(x, y):
					...
					display = "statusmenu"
				if button_day_menu.collidepoint(x, y):
					logging.info('Day button')
					day="yes"
	...
	if day == "yes":
		...
		wifiIcon = "wifi-day.png"
		nowifiIcon = "no-wifi-day.png"
		...
		wifiIcon = "wifi-night.png"
		nowifiIcon = "no-wifi-night.png"
	...
	if display == "countdownmenu":
		...
		# Display countdown
		if countdownenabled == "yes":
			text = fontSm.render("Countdown to "+str(countdownday)+"/"+str(countdownmonth)+"/"+str(countdownyear), True, colourWhite)
	...
	if display == "clock":
		...
		if sync == 0:
			text = fontSm.render("Time out of sync", True, colourRed)
		...
		if connection is True:
			icon = pygame.image.load(icondir+wifiIcon).convert_alpha()
		else:
			icon = pygame.image.load(icondir+nowifiIcon).convert_alpha()
		screen.blit(icon, (280, 5))
